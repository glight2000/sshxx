...
可自建的持久化协作终端：在多页面画布中组织终端、便利贴、文件窗口和自定义组件，通过浏览器或 Tauri 客户端访问同一个工作区；Shell 由独立的
...
| 共享画布         | 携带页面标识的终端、便利贴、文件窗口、自定义组件、布局和在线状态          |
| 结构化便利贴     | 支持段内换行、段落多选/排序、结构化复制、目标关联与发送                   |
| 终端旁的文件能力 | 同步文件夹导航、预览、CodeMirror 编辑、上传和文件操作                     |
| 自定义组件       | 共享 HTML/JavaScript 或 URL 内容、语法编辑器和各浏览端隔离预览            |
| 多种浏览端       | Web 与 Tauri 打包客户端共用同一套 Svelte 界面                             |
| 手机浏览         | 触摸画布、可折叠导航、聚焦阅读/编辑，以及沿用终端配色的阅读区和本地输入框 |
...
**v0.12.0**
新增手机导航和可自定义的桌面快捷键，增强终端输出恢复与粘贴处理，并避免切页时的冗余重绘。参见[模块版本与升级注意事项](https://github.com/glight2000/sshxx/wiki/Installation-and-Releases#v0120-upgrade-notes)。启用随包提供的 terminal-host 修复需要择机重启 host，这会中断其托管进程；仅安装更新不会主动重启它。
...
| Shell 与 SSH 进程                              | `sshxx-terminal-host` 内存                           | 输入输出流共享；浏览端和 daemon 重启后继续；host/系统状态丢失会结束原进程，随后为 SSH 配置窗口启动新的 SSH 进程 |
...
拥有写权限的浏览端也可以在设置中重启 daemon 控制通道，或强制重置 terminal-host 运行态。前者保留 host 中的 PTY；后者会先显示破坏性确认，并终止全部终端进程。若要启用新安装的 terminal-host 二进制版本，应使用上方的 CLI/服务管理器流程，而不是应用内重置。
...
- terminal-host 或操作系统重启会中断托管进程。SSH 配置终端会重新执行其 SSH 启动配置，但默认终端会关闭，Codex

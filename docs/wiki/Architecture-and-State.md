...
copy. The Web server sends `frame-ancestors 'none'` and `X-Frame-Options: DENY`;
the client also refuses to mount when it detects an embedding frame. Together
these rules prevent custom code from recursively rendering an authenticated
sshxx workspace in production, development, and the packaged client. URL mode
also rejects the current viewer origin before navigation. Server validation
bounds each source to 256 KiB, each URL to 4 KiB, the source session total to 4
MiB, and the component count to 100.
